...
export class LeftHalfCommand implements CommandImplementation {
  ...
  impl(): boolean {
    try {
      ...
      console.log(
        `Positioned window to left half: ${newWidth}x${newHeight} at (${newX}, ${newY})`,
      );
      return true;
    } catch (error) {
      console.error("Error in LeftHalf:", error);
      return false;
    }
  }
}
